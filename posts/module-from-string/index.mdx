...
Import Assertions also allows, for example, json format to be treated as a JSON Modules. See [Summary of import assertions and JSON modules](./import-assertions-json-modules) for details.
...
[^1]: [Data URL Syntax](https://developer.mozilla.org/ja/docs/Web/HTTP/Basics_of_HTTP/Data_URIs#syntax)

### Base64 encoding

Expressions in JavaScript modules must be encoded in Base64 because they contain spaces and brackets.

The Web API has long had [btoa](https://developer.mozilla.org/ja/docs/Web/API/btoa). However, it is not a pure function.
An exception is raised if a character that occupies more than 2 bytes in UTF-16 is included. [^2]

[^2]: [Unicode character string](https://developer.mozilla.org/ja/docs/Glossary/Base64)
