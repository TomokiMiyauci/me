...
まずは開発環境の作ります。Node.js やパッケージマネージャーはインストール済みとして進めます。
適当なディレクトリに Vitepress をインストールします。
...
今回は`docs` とします。また、ドキュメントルートに`index.md`を追加します。
...
このタイトルのプロジェクト名の変更については、後述するグローバル設定から変更できます。
...
`lang`プロパティでは`html`タグの`lang`属性を変更できる...かと思いきやできません。
`$siteByRoute`や`$site`といった Vitepress 用の組み込み属性に追加されるので、
`.vue`コンポーネントを作って組み込む際に使うといいでしょう。
...
マークダウンファイルと URL パスにあるように、Vitepress はファイルベースでパスを生成します。
...
`locales`に指定した key が URL パスにマッピングされるようです。
...
`themeConfig`だけではなく、`head`タグも多言語化可能です。例えば、`title`プロパティを多言語対応するには次のようにします。
...
Vitepress でドキュメントの作成を見てきました。簡単にドキュメントを作成できると感じられたのではないでしょうか。

現在、Vitepress はミニマムということにかなりこだわっており、その役割をドキュメントの生成に専念させるのか、ブログ用などの機能の追加を行うのか議論されています。
